...
use App\Models\Back\Catalog\ProductImageTranslation;
use Carbon\Carbon;
...
class ProductImage extends Model
{
    ...
    {
        $path = Image::save('product', $new_image, $this->resource);

        $id = $this->insertGetId([
            'product_id' => $this->resource->id,
            'image'        => config('filesystems.disks.product.url') . $path,
    ...
    public function replace(int $id, array $image)
    {
        ...
        $path = Image::cleanPath('product', $this->resource->id, $old->image);

        // Obriši staru i snimi novu fotku
        Image::delete('product', $this->resource->id, $path);
        $new_path = Image::save('product', $image, $this->resource);

        $updated = $this->where('id', $id)->update([
            'image' => config('filesystems.disks.product.url') . $new_path
        ]);
